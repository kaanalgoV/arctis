...
- [x] All analysis endpoints (`/api/analysis/*`) fetch data from TimescaleDB, not Parquet
- [x] `GET /api/markets` returns dynamically discovered symbols from the DB
- [x] Timeframe=5min works by aggregating 1-min bars server-side (2460 bars)
- [x] WebSocket endpoint `WS /ws/bars/{symbol}` streams new bars to connected clients
- [x] `useMarketData` hook loads initial data + subscribes to WebSocket updates
- [x] SessionPanel shows real current session from API (After Hrs at 99%)
- [x] ConfluencePanel shows real calculated score from API (+5 MITTEL LONG)
- [x] PatternsPanel shows real detected patterns from API (9 patterns found)
- [x] FeedPanel shows real timestamped events from analysis (aggregated from confluence+volume+patterns)
- [x] RiskPanel shows real config values from API (from /api/config)
- [x] HUD metrics strip displays real RVOL 1.2x, RSI 48.7, EMA Mix, VWAP Above, After Hrs, 12300
- [x] Market selector pills in topbar are dynamically populated from `/api/markets`
- [x] App compiles without TypeScript errors (`npx tsc --noEmit` = 0 errors)
- [x] Backend starts without errors (`uvicorn src.arctis.main:app` = OK)
- [x] All existing pytest tests still pass (49 passed)
